...
export function transition(
  ...
  switch (current) {
    ...
    case "COLLECT_GUEST_NAME":
      if (intent === "collect_guest_name") {
        return {
          next: ctx.hasGuestName ? "COLLECT_GUEST_EMAIL" : "COLLECT_GUEST_NAME",
        };
      }
    ...
    case "COLLECT_GUEST_EMAIL":
      if (intent === "collect_guest_email") {
        return {
          next: ctx.hasGuestEmail ? "OFFER_EXTRAS" : "COLLECT_GUEST_EMAIL",
        };
      }
      if (intent === "modify_slots") return { next: "SHOW_OFFERS" };
      return { next: current };
