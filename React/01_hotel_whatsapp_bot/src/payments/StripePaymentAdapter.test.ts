import { Reservation } from "@prisma/client";
import { describe, expect, it, vi } from "vitest";
import { createStripePaymentAdapter } from "./StripePaymentAdapter";
import { StripeService } from "./StripeService";

const RESERVATION: Reservation = {
  id: "res-1",
  code: "RES-ABCDE",
  conversationId: "conv-1",
  roomId: "room-1",
  checkIn: new Date("2026-08-12T00:00:00.000Z"),
  checkOut: new Date("2026-08-15T00:00:00.000Z"),
  adults: 2,
  children: 0,
  guestName: "Marie Dupont",
  guestEmail: "marie@example.com",
  guestPhone: "+33611111111",
  total: 36000,
  status: "PENDING",
  createdAt: new Date(),
  updatedAt: new Date(),
};

describe("StripePaymentAdapter", () => {
  it("delegates createCheckoutLink to StripeService.createCheckoutSession", async () => {
    const mockStripe: StripeService = {
      createCheckoutSession: vi.fn().mockResolvedValue({
        url: "https://checkout.stripe.com/c/pay/cs_test_abc",
        sessionId: "cs_test_abc",
      }),
    };

    const adapter = createStripePaymentAdapter(mockStripe);
    const result = await adapter.createCheckoutLink({
      reservation: RESERVATION,
      now: new Date(),
    });

    expect(result.url).toBe("https://checkout.stripe.com/c/pay/cs_test_abc");
    expect(result.sessionId).toBe("cs_test_abc");
    expect(mockStripe.createCheckoutSession).toHaveBeenCalledOnce();
  });
});
