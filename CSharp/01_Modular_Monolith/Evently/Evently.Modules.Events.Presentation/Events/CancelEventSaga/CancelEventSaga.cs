...
public sealed class CancelEventSaga : MassTransitStateMachine<CancelEventState>
{
    ...
    public CancelEventSaga()
    {
            ...
            When(EventCanceled)
                .Publish(context =>
                    new EventCancellationStartedIntegrationEvent(
                        ...
                        context.Message.EventId)).TransitionTo(CancellationStarted));
        ...
        During(PaymentsRefunded, When(EventTicketsArchived).TransitionTo(TicketsArchived));

        During(TicketsArchived, When(EventPaymentsRefunded).TransitionTo(PaymentsRefunded));
